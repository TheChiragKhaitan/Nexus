...
const ColorButton = ({onClick, color}: ColorButtonProps) => {
    return (
        <button 
            ...
            type="button"
            className="w-8 h-8 items-center justify-center flex transition hover:opacity-75"
            onClick={() => onClick(color)}
        >
            <div className="h-8 w-8 border border-neutral-300 rounded-md" style={{ background: colorToCss(color)}} />
        </button>
    )
}