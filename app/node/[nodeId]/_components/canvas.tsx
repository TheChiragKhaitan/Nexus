...
const Canvas = ({boardId}: CanvasProps) => {
    ...
    const [lastUsedColour, setLastUsedColour] = useState<Color>({r:0 , g:0, b:0})
    ...
    const startMultiSelection = useCallback((current: Point, origin: Point) => {
        ...
        if (Math.abs(current.x - origin.x) + Math.abs(current.y - origin.y) > MULTISELECTION_THRESHOLD) {
            setCanvasState({ mode: CanvasMode.SelectionNet, origin, current})
        }
    ...
    }, [canvasState, resizeSelectedLayer, continueDrawing, camera, translateSelectedLayer, startMultiSelection, updateSelectionNet])
    ...
    }, [camera, setCanvasState, canvasState, history, insertLayer, insertPath, unselectLayer])
                ...
                <g style={{transform: `translate(${camera.x}px, ${camera.y}px)`}}>
                    {layerIds.map((layerId) => (
                        <LayerPreview
                            ...
                            selectionColour = {layerIdsToColorSelection[layerId]} 
                        />
                    ))}
